...
usage() {
  cat <<'EOF'
Usage: create_user.sh [--api-url URL] [--admin-user USER] [--admin-password PASS] [--enabled true|false] USERNAME [PASSWORD]
EOF
}
...
if [[ -z "$password" ]]; then
  password="$(ts5_prompt_secret "Password for $username")"
fi
...
payload="$(jq -n --arg username "$username" --arg password "$password" --argjson enabled "$enabled" '{username: $username, password: $password, enabled: $enabled}')"

response="$(ts5_api_call "$api_url" POST '/proxy-users' "$token" "$payload")"
printf '%s\n' "$response" | jq .
